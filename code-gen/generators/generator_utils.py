...
def generic_generate_func_impl(
    ...
    if strategy != "reflexion" and strategy != "simple" and strategy != "cot":
        raise ValueError(
            f"Invalid strategy: given `{strategy}` but expected one of `reflexion` or `simple` or `cot`.")
    ...
    if model.is_chat:
        if strategy == "reflexion":
            ...
            func_bodies = model.generate_chat(messages=messages, num_comps=num_comps, temperature=temperature)
        elif strategy == "cot": # chain of thought prompting
            system_prompt = f"{cot_chat_instruction}\n{code_block_instruction}"
            print_messages(system_prompt, func_sig)
            messages = [
                Message(
                    role="system",
                    content=f"{cot_chat_instruction}\n{code_block_instruction}",
                ),
                Message(
                    role="user",
                    content=func_sig,
