...
from utils import read_jsonl, read_jsonl_gz, load_evalplus_dataset
from mcts import run_mcts
from dfs import run_dfs
from multi_agent_lessons_opensource import run_multi_agent_lessons_opensource
from moa import run_moa_gpt4o_aggregator
...
def get_args():
    ...
    parser.add_argument("--strategy", type=str,
                        help="Strategy: `simple`, `reflexion`, `cot`")
    parser.add_argument("--language", type=str, help="Strategy: `py` or `rs`")
    parser.add_argument(
        "--model", type=str, help="A place holder argument for LessonL. OpenAI and VLLM models only for now.")
...
def strategy_factory(strategy: str):
    ...
    if strategy == "simple":
        return kwargs_wrapper_gen(run_simple, delete_keys=["expansion_factor", "max_iters", "number_of_tests"])
    elif strategy == "cot":
        return kwargs_wrapper_gen(run_simple, delete_keys=["expansion_factor", "max_iters", "number_of_tests"])
    ...
    elif strategy == "lesson-opensource":
        return kwargs_wrapper_gen(run_multi_agent_lessons_opensource, delete_keys=["expansion_factor"])
    elif strategy == "moa":
        return kwargs_wrapper_gen(run_moa_gpt4o_aggregator, delete_keys=["expansion_factor"])
    else:
...
""")
    ...
    # Check if dataset_path is a benchmark name (humaneval or mbpp) or a file path
    if args.dataset_path.lower() in ['humaneval', 'mbpp']:
        # Use evalplus to load the dataset
        dataset = load_evalplus_dataset(args.dataset_path)
        print(f"Loaded {len(dataset)} examples from evalplus {args.dataset_path}")
    ...
    #     print(f"Loaded {len(dataset)} examples")
        ...
        raise ValueError(
            f"Dataset path `{args.dataset_path}` is not supported. Use 'humaneval' or 'mbpp'")
    # start the run
    if args.strategy == "lesson-opensource":
