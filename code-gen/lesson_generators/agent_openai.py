...
class VLLMAgent(AbstractLLMAgent):
    ...
    def generate_prompt(self, context: dict, task: str) -> str:
        ...
        if task == "CODE":
            if context["lessons"] == "":
                prompt = generate_code_opt_prompt_code(src_code=context["src_code"])
            else:
                prompt = generate_code_opt_prompt_code_with_lessons(src_code=context["src_code"], lessons=context["lessons"])
        elif task == "LESSON":
    ...
    def generate_code_with_other_codes(self, context: dict, other_codes: str) -> str:
        """
        Perform Code Optimization by using LLM
        ...
        prompt = generate_code_opt_with_other_responses(src_code=context["src_code"], other_codes=other_codes)
        # prompt = self.generate_prompt(context=context, task="CODE")
        #self.logger.info("Optimizer Prompt: \n%s", prompt)
        #self.logger.info("OPTIMIZER PROMPT END!!!")
        output, in_tokens, out_tokens = self.infer(prompt=prompt)
    ...
    def generate_code_with_hint(self, context: dict, other_codes: str) -> str:
        """
        Perform Code Optimization by using LLM
